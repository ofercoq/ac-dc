...
int main(int argc, char * argv[]) {
  ...
  if (dense) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int j = part.A_csr_row_ptr[i]; j < part.A_csr_row_ptr[i + 1];
	   j++) {
	Li[i] += part.b[i] * part.b[i] * part.A_csr_values[j] * part.A_csr_values[j];
      }
      if (Li[i] > 0) {
  ...
  for (int PM = 0; PM < 20; PM++) {
	...
	for (int i = 0; i < n; i++) {
	  y[j] += A[i * m + j] * part.b[i] * LiSqInv[i] * x[i];
	}
      }
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (dense) {
	for (int j = 0; j < m; j++) {
	  x[i] += A[i * m + j] * part.b[i] * LiSqInv[i] * y[j];
	}
      } else {
	for (int j = part.A_csr_row_ptr[i];
	     j < part.A_csr_row_ptr[i + 1]; j++) {
